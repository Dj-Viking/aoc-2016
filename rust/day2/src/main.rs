...
fn part2() {
	...
	// starting point is at 5 on the new keypad
	...
	let grid_file = read_to_string("./samplekeypad2")
		.unwrap();
	...
	let grid_with_start = "[".to_string() + &grid[position.0][position.1] + "]";
	grid[position.0][position.1] = grid_with_start; 
	...
	for row in 0..dirs.len() {
		...
		for col in 0..chars.len() {
			...
			let previous_pos_zero = position.0.clone();
			position.0 = {
			...
			let previous_pos_one = position.1.clone();
			position.1 = {
			...
			// hmm
			// println!("pos now {:?}", position);
			// println!("previous0 {:?}", previous_pos_zero);
			// println!("previous1 {:?}", previous_pos_one);
			if grid[position.0][position.1].is_empty() {
				position.0 = previous_pos_zero;
				position.1 = previous_pos_one;
			}
			...
			// if on the last instruction of the instruction row
...
fn part1() {
	...
	// this needs to still be rows of letters for directions to the next number of the door
	// code 
	let dirs = instructions
		.split("\n")
		.filter(|x| !x.is_empty())
